...
use AppBundle\Repository\StuffRepository;
...
class StuffController extends Controller
{
     ...
     * @param $name
     * @return JsonResponse
     * @Route("/list/{name}", name="list_stuff")
     */
    public function autocompleteAction(request $request, $name)
    {
        if ($request->isXmlHttpRequest()) {
            /**
             *  @var $repository StuffRepository
             */
            $repository = $this->getDoctrine()->getRepository('AppBundle:Stuff');
            $data = $repository->getStuffLike($name);
