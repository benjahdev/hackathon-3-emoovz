...
{
    /**
     * @Route("/add/{id}/{room_id}", name="add_stuff")
    ...
    public function addStuffToInventoryAction(Request $request, Stuff $stuff, Room $room)
    {
        if ($request->isXmlHttpRequest()) {
            ...
            } else if (!$stuffBasket->containRooms($room)) {
                $stuffBasket->addRoom($room);
            }
            ...
            if (!$inventory->containStuff($stuffBasket)) {
